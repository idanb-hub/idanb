...
# Module-level state shared between Part 2 (ExtractIOCs) and Part 3 (FlowSearch).
# When the user clicks "Use selected IOCs", _extracted_iocs is updated and
# _iocs_version is bumped; FlowSearch observes the counter to re-populate.
    ...
    *,
    ...
    react.use_effect(
        lambda: (
            set_state(
                addresses=tuple(
                    v.split()[0] for v, _ in effective_iocs.ip_addresses
                ),
                domains=tuple(v for v, _ in effective_iocs.domains),
                urls=tuple(v for v, _ in effective_iocs.urls),
            )
